                    ...
                    firebaseAuth.getCurrentUser().sendEmailVerification().addOnCompleteListener(new OnCompleteListener<Void>() {
                        ...
                        public void onComplete(@NonNull Task<Void> task) {
                            if (task.isSuccessful()){
                                String abc = firebaseAuth.getUid();
                                FirebaseDatabase database = FirebaseDatabase.getInstance();
                                DatabaseReference Users = database.getReference("Hotels").child(Phone);
                                DatabaseReference pins = database.getReference("Hotel Pins").child(Pin);
                                pins.child(abc).setValue("("+Phone+")"+" "+Name);
                                Users.child("Email").setValue(Email);
                                Users.child("Name").setValue(Name);
                                Users.child("Pin").setValue(Pin);
                                Users.child("Phone").setValue(Phone);
